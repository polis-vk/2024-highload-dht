## Этап 1
При одновременном запуске wrk2 и async-profiler мой серв
ер выдерживает нагрузку в 6500 rps на GET запросы
...
Протестируем PUT запросы

[put_t4_c70_R22000.txt](wrk%2Fresults%2Fstage4%2Fput_t4_c70_R22000.txt) \
_Сервер продолжает выдерживать 22000 RPS при 70 соединениях_

[put_t4_c170_R6000.txt](wrk%2Fresults%2Fstage4%2Fput_t4_c170_R6000.txt) \
_Север с трудом выдерживает 6000 RPS при 170 соединениях_

[put_t4_c170_R5000.txt](wrk%2Fresults%2Fstage4%2Fput_t4_c170_R5000.txt) \
_Сервер выдерживает 5000 RPS при 170 соединениях_

Профилирование будем проводить при 70 соединениях и 22000 RPS

![put_alloc_c70_R22000.png](asprof%2Fstage4%2Fput_alloc_c70_R22000.png)
[put_alloc_c70_R22000.html](asprof%2Fstage4%2Fput_alloc_c70_R22000.html) \
_Появились накладные расходы на сериализацию, которую я использую, чтобы сохранять timestamp_

![put_cpu_c70_R22000.png](asprof%2Fstage4%2Fput_cpu_c70_R22000.png)
[put_cpu_c70_R22000.html](asprof%2Fstage4%2Fput_cpu_c70_R22000.html) \
_Увеличились затраты на общение между нодами; они действительно теперь общаются чаще_

![put_lock_c70_R22000.png](asprof%2Fstage4%2Fput_lock_c70_R22000.png)
[put_lock_c70_R22000.html](asprof%2Fstage4%2Fput_lock_c70_R22000.html) \
_Увеличились затраты на общение между нодами_

Протестируем GET запросы

[get_t4_c50_R6500.txt](wrk%2Fresults%2Fstage4%2Fget_t4_c50_R6500.txt) \
_Получаем огромные тайминги при 6500 RPS и 50 соединениях_

[get_t4_c50_R3500.txt](wrk%2Fresults%2Fstage4%2Fget_t4_c50_R3500.txt) \
_При 3500 RPS и 50 соединениях все еще большие тайминги_

[get_t4_c30_R3000.txt](wrk%2Fresults%2Fstage4%2Fget_t4_c30_R3000.txt) \
_При 3000 RPS и 30 соединениях тайминги значительно уменьшились, но все еще большие_

[get_t4_c30_R2500.txt](wrk%2Fresults%2Fstage4%2Fget_t4_c30_R2500.txt) \
_Запросы отрабатывают за адекватное время при 2500 RPS и 30 соединениях_

Профилирование будем проводить при 30 соединениях и 25000 RPS

![get_alloc_c30_R2500.png](asprof%2Fstage4%2Fget_alloc_c30_R2500.png)
[get_alloc_c30_R2500.html](asprof%2Fstage4%2Fget_alloc_c30_R2500.html) \
_Сильно увеличилось число аллокаций внутри ```seekForValueInFile```.
Это можно объяснить тем, что теперь мы несколько раз обращаемся к файлу для получения значений из реплик_

![get_cpu_c30.png](asprof%2Fstage4%2Fget_cpu_c30.png)
[get_cpu_c30_R2500.html](asprof%2Fstage4%2Fget_cpu_c30_R2500.html) \
_Сильно увеличились затраты на ```seekForValueInFile```. Объяснение аналогично вышеописанному_

![get_lock_c30_R2500.png](asprof%2Fstage4%2Fget_lock_c30_R2500.png)
[get_lock_c30_R2500.html](asprof%2Fstage4%2Fget_lock_c30_R2500.html) \
_Уменьшилась доля ```HttpSesssion.SendResponse```. Действительно, общение по сети с клиентом сейчас не самое страшное_

Вывод: добавление репликации с точки зрения производительности сервера несильно сказалось на PUT запросах 
(увеличились накладные расходы на общение между нодами) и сильно ухудшило производительность GET запросов 
(они и так были слабым местом, а теперь приходится искать вхождение в файле несколько раз) 