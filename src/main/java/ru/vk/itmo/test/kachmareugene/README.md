...
Первое, что бросалось в глаза, так это то,
что при некоторых колеблились персентили. 

При put могли быть порядка 200, а то и 1000. 
Перед каждым put перезапускался сервер.
С get'ом такого разброса не наблюдалось.

## CPU

[PUT](results/task5/putCPU.html)
[GET](results/task5/getCPU.html)

В целом мы видим, что втисалось в нагрузку логика с CompletableFuture.
И она занимает 36.5%(67.5(*) - 31(**)), когда остальная обработка запросов 31%.

-* смотрю на ThreadPoolExecutor.runWorker
-** обычные запросы обработки

Похожая история с PUT, но больший процент занятости проц. время перешло к фьючерсам.

## ALLOC

[PUT](results/task5/putALLOC.html)
[GET](results/task5/getALLOC.html)

Полявилась сложная иерархия аллоков. 
Не удивительно, ведь при ассинхронных операциях мы убираем вычисления на 2ой план, 
то есть кладем их в память и далее лениво достаем и обсчитываем.

Примерно 2/3 у PUT и GET занимают операции с фьючерсами,
где 1/3 приходится на старые вызовы функций аллоцирующих память.


## LOCK

[PUT](results/task5/putLOCK.html)
[GET](results/task5/getLOCK.html)

Старый ThreadPoolExecutor.getTask в текущих реалях начал быть чересчур малым,
что с трудом можно найти его на диаграмме. 

Вывод: накладные расходы со всех сторон получились достаточно объемными, добавление асинхронного кода, к сожалению, не панацея.