...
public abstract class ReplicativeBaseSharder extends BaseSharder {
    ...
    private Response chooseResponse(List<Response> responses, Set<Integer> validStatuses) {
        return responses.stream()
                .filter(response -> validStatuses.contains(response.getStatus()))
                .max(timestampComparator).orElse(new Response(Response.INTERNAL_ERROR, Response.EMPTY));
    }

    private Response makeMethodDecision(List<Response> responses, int acknowledge, Set<Integer> validStatuses) {
        if (isSuccessful(responses, acknowledge, validStatuses)) {
            return chooseResponse(responses, validStatuses);
        }
    ...
    public Response makeDecision(List<Response> responses, int acknowledge, int method) {
        return switch (method) {
            case Request.METHOD_GET -> makeMethodDecision(responses, acknowledge, Set.of(200, 404));
            case Request.METHOD_PUT -> makeMethodDecision(responses, acknowledge, Set.of(201));
            case Request.METHOD_DELETE -> makeMethodDecision(responses, acknowledge, Set.of(202));
