...
-  SynchronousQueue - содержит только один элемент - что тоже не подходит нам 
...
Быстрее оказалась ArrayBlockingQueue использует внутренний массив для хранения элементов, что обеспечивает более эффективный доступ к элементам в памяти.
...
По результатам тестирования было получено что при длине очереди большей чем 300 практически не встречаются ошибки, при этом при длине очереди 100 ошибки встречаются часто. Поэтому было принято решение использовать длину очереди 500, то етсь с небольшим запасом.
