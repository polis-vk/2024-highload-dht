...
* Максимальный размер очереди - слишком большая очередь будет позволять храниться старым запросам, которые мы, возможно, уже не ждем. 
...
* Максимальное время обработки запроса - зависит от SLA предъявляемых вашему сервису. В своей реализации я добавил
  игнорирование запросов, у которых истек таймаут (сейчас стоит 200 мс). На такие запросы клиенту отправляется ответ "
  408 Request Timeout".
...
* AbortPolicy кидает исключение, которое мы можем обработать, чтобы оповестить наших клиентов о перегрузке сервера - вариант, который выбран сейчас
