...
final class TableSet {
    ...
    Iterator<TimestampEntry<MemorySegment>> get(
        ...
        for (int i = 0; i < ssTables.size(); i++) {
            ...
            if (ssTableIterator.hasNext()) {
                iterators.add(new WeightedPeekingEntryIterator(i, ssTableIterator));
            }
        }
    ...
    TimestampEntry<MemorySegment> get(final MemorySegment key) {
        ...
        // Nothing found
        return new TimestampBaseEntry<>(key, null, 0L);
    }

    private static TimestampEntry<MemorySegment> swallowTombstone(final TimestampEntry<MemorySegment> entry) {
        return entry.value() == null ? new TimestampBaseEntry<>(entry.key(), null, entry.timestamp()) : entry;
    }
    ...
    Iterator<TimestampEntry<MemorySegment>> allSSTableEntries() {
        ...
        for (int i = 0; i < ssTables.size(); i++) {
            ...
            iterators.add(
                    new WeightedPeekingEntryIterator(i, ssTableIterator));
        }
